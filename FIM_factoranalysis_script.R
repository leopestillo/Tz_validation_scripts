...
data_test<-with(data_validation,data.frame(fam_income,personal_income))

data_test_imputed <- mice(data_test, seed = 2222, m=10)
data_test<-mice::complete(data_test_imputed,4)
...
#FIM
FIM_data18<-with(data_validation,data.frame(g1,g2,g3,g4,g5,g6,g8,g9,g10,g11,g12,g14,g15,g17,g18,g22,g26,g27))

# argument method=c("") indicated the imputation system (see Table 1 in http://www.jstatsoft.org/article/view/v045i03). Leaving "" to the position of the variable in the method argument excludes the targeted variable from the imputation.
data_imputed_18 <- mice(FIM_data18, seed = 2222, m=10)

# reports the complete dataset with missing imputated. It returns 5 options of datasets, witht he 5 imputation possibilities. To choose a specific option, add # as argument. Ex. complete(imp,2)
FIM_data18<-mice::complete(data_imputed_18,4)

#FIM Motor
FIM_Motor18<-with(FIM_data18,data.frame(g1,g2,g3,g4,g5,g6,g8,g9,g10,g11,g12,g14,g15))

#FIM Cognitive
FIM_Cognitive18<-with(FIM_data18,data.frame(g17,g18,g22,g26,g27))
...
psych::alpha(FIM_data18,n.iter=1000,check.keys=TRUE)
psych::alpha(FIM_Motor18,n.iter=1000,check.keys=TRUE)
psych::alpha(FIM_Cognitive18,n.iter=1000,check.keys=TRUE)
...
sum(Est$std.all[1:13])^2/(sum(Est$std.all[1:13])^2+sum(Est$std.all[127:139]))
sum(Est$std.all[14:18])^2/(sum(Est$std.all[14:18])^2+sum(Est$std.all[140:144]))

#Average Extracted Variance
sum(Est$std.all[1:13]^2)/length(Est$std.all[127:139])
sum(Est$std.all[14:18]^2)/length(Est$std.all[140:144])
...
# 2 factor model - I don't remember why I did it
cfa_model <- '
...
# without g7
cfa_model <- '
MF =~ g1 + g2 + g3 + g4 + g5 + g6 + g8 + g9 + g10 + g11 + g12 + g13 + g14 + g15 + g16 
CF =~ g17 + g18 + g20 + g21 + g22 + g23 + g24 + g25 + g26 + g27 + g28 + g29 + g30
'
...
#Composite Reliabilty
#MF
sum(Est$std.all[1:16])^2/(sum(Est$std.all[1:16])^2+sum(Est$std.all[211:226]))
#CF
sum(Est$std.all[17:30])^2/(sum(Est$std.all[17:30])^2+sum(Est$std.all[227:240]))


#Average Extracted Variance
sum(Est$std.all[1:16]^2)/length(Est$std.all[1:16])
sum(Est$std.all[17:30]^2)/length(Est$std.all[17:30])

#Thresholds
b#see correct lines 
y(Est$std.all[13:50],Est$lhs[13:50],mean)
...
#Factor scores
Factorscorres_FIM<-lavaan::predict(fit)

write.csv(Factorscorres_FIM, "/Users/LeoPestillo/Desktop//FIM_data.csv")


#external validation

data<-read.csv("/Users/LeoPestillo/Google Drive/[PES] life discussions/Tz_Validations/Data_correlation_FIM.csv",sep=',')
